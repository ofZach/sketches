...
public: 
    ...
    void draw(float time, float distanceFromTime, float totalTime){
        ...
        /*
         ...
         start with a stroke.  If the first point is in time -- keep going until we hit a non in time point, then loop to that point, alternating drawing. 
         
         If all points are in time, draw everything. 
         
         */
        
        if (timePts.size() < 2) return; // we can't do nothing for ya man. 
            
        
        bool bHitNonInTime = false;
        int whichIsNotInTime = -1;
        ...
        if (!bHitNonInTime){
            ...
            ofPolyline line;
            for (int i = whichIsNotInTime; i < (whichIsNotInTime+timePts.size()); i++){
                int who = i % timePts.size();
                if (timePts[who].time >= startTime && timePts[who].time <= time){
                    line.addVertex(timePts[who].pt);
                } else {
        ...
        // this is to deal with the line which wrap of t = 0;
...
class rotoSketchScene : public baseScene {
